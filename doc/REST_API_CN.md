...
- [现货(Spot)业务API参考](#现货spot业务api参考)
    ...
    - [币币账户API](#币币账户api)
        ...
        - [4. 撤销所有委托](#4-撤销所有委托)
        - [5. 按订单撤销委托](#5-按订单撤销委托)
        - [6. 按订单批量撤销委托](#6-按订单批量撤销委托)
        - [7. 查询当前未成交订单](#7-查询当前未成交订单)
        - [8. 通过id获取订单](#8-通过id获取订单)
        - [9. 获取币对历史成交记录，支持分页查询](#9-获取币对历史成交记录，支持分页查询)
        - [10. 通过订单类型查询所有订单](#10-通过订单类型查询所有订单)
        - [11. 获取账单](#11-获取账单)
<!-- /TOC -->
# 介绍
欢迎使用[康波]()开发者文档。
...
|start|String|否|开始时间（unix时间戳,单位ms）|
|end|String|否|结束时间（unix时间戳,单位ms）|
...
|-----|:---:|----|----|
|pairCode|String|是|币对，如LTC_BTC|
|status|Integer|是|0:未成交 1:部分成交 2:完全成交 -1:已撤单|
|startDate|Long|否|开始时间，如1524801032573|
|endDate|Long|否|结束时间，如1524801032573|
|systemOrderType|Integer|否|10:限价 11:市价|
|price|BigDecimal|否|价格|
